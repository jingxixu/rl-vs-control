...
def get_args():
    ...
    parser.add_argument('--model_fnm', type=str)
    ...
    args.perception_model_path = mu.get_perception_model_path(args.ob_type, args.fixation, args.rgb, args.resolution) if args.pnoise else None
    if args.model_fnm == None:
        args.fixstr = mu.convert_float_to_fixstr(args.fixation)
        args.model_fnm = 'robust_control/params/hinf/hinf_'+args.fixstr+'_obtype_'+args.ob_type
        if args.pnoise:
...
def enjoy(fixation = 1.0, pnoise = False, rgb = False, ob_type = 'z_sequence', num_traj = 3000, trial_number=1, trial_type="", delay = 0):
    args = Argument()
    ...
    args.fixstr = mu.convert_float_to_fixstr(args.fixation)
    args.model_fnm = 'control_experiment_data/trial'+trial_type+str(trial_number)+'/params/hinf/hinf_'+args.fixstr+'_obtype_'+args.ob_type
    if args.pnoise:
        args.model_fnm += '_pnoise'
        if args.rgb:
            args.model_fnm += '_rgb'
    ...
    for ep in range(n_trials):
        # state = env.reset(randstate=np.zeros(4))
        state = env.reset()
        controller.reset()
        ...
        for i in range(500):
            if args.ob_type == 'full':
                ...
                action = controller.control(state[-1])
            if i < Nx:
                action = 0
